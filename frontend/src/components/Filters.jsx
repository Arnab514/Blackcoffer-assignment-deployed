...
const Filters = ({ setMainData }) => {
    ...
    const getDataFromDB = async(year) => {
        try{
            const response = await axios.get(`https://arnab-blackcoffer-assignment.vercel.app/api/data/year/${year}`);
    ...
    const handleReset = async() => {
        try{
            const response = await axios.get("https://arnab-blackcoffer-assignment.vercel.app/api/data/all");
            setMainData(response.data.data)
        }
        catch(e){
            console.log(e)
